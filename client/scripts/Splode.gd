...
var NHHC = preload("res://godot/scenes/interface/weapons/FSL 23837.tscn")
...
func damage(amount: int, transform: Transform):
	...
	nhhc.apply_central_impulse(Vector3(rand_range(-20, 20), rand_range(10, 15), rand_range(-20, 20)))
	ar.global_transform.origin = global_transform.origin
	ar.apply_central_impulse(Vector3(rand_range(-20, 20), rand_range(10, 15), rand_range(-20, 20)))
	fsl.global_transform.origin = global_transform.origin
	fsl.apply_central_impulse(Vector3(rand_range(-20, 20), rand_range(10, 15), rand_range(-20, 20)))
	pump.global_transform.origin = global_transform.origin
	pump.apply_central_impulse(Vector3(rand_range(-20, 20), rand_range(10, 15), rand_range(-20, 20)))
	sus.global_transform.origin = global_transform.origin
	sus.apply_central_impulse(Vector3(rand_range(-20, 20), rand_range(10, 15), rand_range(-20, 20)))
